...
; Comments:
;
Pro amp_calc_rotmat_sec, r0, CRLat, RLon, south, rot_mat
; If south = 1 then rotate to south hemisphere intersection else use Nth hemisphere
; changed to include r0 to be passed in - CLW, Nov 2014
;
; Error check
  status=0
  catch,error_status
  if (error_status ne 0) then begin
    status=1
    Print,'##ERROR : amp_calc_rotmat -> returning...'
...
;  r0=6371.0+780.0 ; km
	...
	_r0=r0 ; This fixed Colin's bug 
    If south then _r0=-r0                                            ; point r vec along Z axis in correct direction for Hemis

; cross product (x,y,z) so for south->(0,0,-r) is the pole
    cross_p,[x_sh,y_sh,z_sh],[0.0,0.0,_r0],v_rot                     ; rotation is about this axis, v_rot should always have a zero z component
    c_arg=(_r0*z_sh)/(sh_mag*abs(_r0))
